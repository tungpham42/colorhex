...
export default function ImagePaletteExtractor() {
  ...
  const handleExtractPalette = async () => {
    ...
    try {
      ...
      setError("Error extracting colors. Please try with a different image.");
    } finally {
  ...
  const exportCSSVariables = (download = false) => {
    ...
    if (download) {
      try {
        ...
        setError("Error downloading CSS file. Please try again.");
        setShowToast(true);
        setTimeout(() => setShowToast(false), 2000);
      }
    } else {
  ...
  const navigateToColor = (hexColor: string) => {
    try {
      console.log("🎨 Navigating to color:", hexColor);
      const colorName = hexColor.replace("#", "").toLowerCase();
      const colorUrl = `/${colorName}`;
        ...
        <Col md={8}>
          {imagePreview ? (
            ...
            <Card className="h-100 d-flex align-items-center justify-content-center">
              <Card.Body className="text-center text-muted w-100 align-content-center">
